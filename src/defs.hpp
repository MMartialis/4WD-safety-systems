// defs.hpp

#ifndef DEFS_HPP
#define DEFS_HPP

#include <cstdint>

#define VERBOSE 0
#define VERBOSE_PWM 0

#define BT_LOG 1
#define BT_LOG_TIMESTAMP 1

#define MAIN_LOOP_DELAY_US 3000 // microseconds
#define MAIN_LOOP_DELAY_TICKS (MAIN_LOOP_DELAY_US * configTICK_RATE_HZ) / 1000000

#define PIPI PI * PI

#define FL_ID 53 // 0x35
#define FL_MAX_CURRENT 60
#define FL_MAX_BRAKE_CURRENT 75
#define FR_ID 77 // 0x4D
#define FR_MAX_CURRENT 60
#define FR_MAX_BRAKE_CURRENT 75
#define RL_ID 13 // 0x0D
#define RL_MAX_CURRENT 90
#define RL_MAX_BRAKE_CURRENT 90
#define RR_ID 40 // 0x28
#define RR_MAX_CURRENT 90
#define RR_MAX_BRAKE_CURRENT 90

#define CAN0_INT 21
#define CAN0_INT_PIN GPIO_NUM_21
#define CAN0_CS 5
#define RX_MSG_BUFFER_LEN 8
#define STATUS_1_COMMAND_ID 9

#define PWM_PIN 2
#define PWM_PIN_PIN GPIO_NUM_2
#define PWM_MIN_INTERVAL_MICROS 1108
#define PWM_MAX_INTERVAL_MICROS 1878
#define PWM_MEDIAN_INTERVAL_MICROS 1498
#define PWM_DEADZONE 25 // the deadzone for the pwm signal in microseconds
#define PWM_WAS_NOT_ZERO_THRESHOLD 2

#define SD_LOG_ENTRY_SIZE 32
#define SD_CS_PIN 17

#define MOTOR_EFFICIENCY 0.85
#define WHEEL_RADIUS 0.095                     // meters
#define WHEEL_DIAMETER WHEEL_RADIUS * 2        // meters
#define WHEEL_CIRCUMFERENCE WHEEL_DIAMETER * PI // meters
#define POLE_PAIR_COUNT 7
#define GEAR_RATIO_FRONT 3.6
#define GEAR_RATIO_REAR 5.2


#define RATIO_F2R_L GEAR_RATIO_FRONT*KV_FL/GEAR_RATIO_REAR/KV_RL
#define RATIO_F2R_R GEAR_RATIO_FRONT*KV_FR/GEAR_RATIO_REAR/KV_RR
#define RATIO_R2F_L GEAR_RATIO_REAR*KV_RL/GEAR_RATIO_FRONT/KV_FL
#define RATIO_R2F_R GEAR_RATIO_REAR*KV_RR/GEAR_RATIO_FRONT/KV_FR

#define ERPM_TO_MPS_FRONT WHEEL_CIRCUMFERENCE / (POLE_PAIR_COUNT * GEAR_RATIO_FRONT * 60)
#define ERPM_TO_MPS_REAR WHEEL_CIRCUMFERENCE / (POLE_PAIR_COUNT * GEAR_RATIO_REAR * 60)
#define AXLE_WIDTH 0.338               // meters
#define HALF_AXLE_WIDTH AXLE_WIDTH / 2 // meters
#define ERPM_TO_MOTOR_RPM 1 / POLE_PAIR_COUNT

// if csúszik v1
#define MAGIC_TRESHOLD 0.5               // m/s
#define IS_SLIDING_THRESHOLD_FRONT 0.2   // unit: I have no idea, please ask about this in the exam :D
#define IS_SLIDING_THRESHOLD_REAR 0.2    // unit: I have no idea
#define REAR_TRACTION_CONTROL_TIMEOUT 0  // milliseconds
#define FRONT_TRACTION_CONTROL_TIMEOUT 0 // milliseconds

// if csúszik v2
#define MAX_AXLE_DIFFERENCE 0.5 // m/s
#define MIN_PWM_THRESHOLD 0.1   // hecto%

// don't v2
#define ANGULAR_MOMENTUM_FL 0.00075 // kg*m^2  // θ
#define ANGULAR_MOMENTUM_FR 0.00075 // kg*m^2  // θ
#define ANGULAR_MOMENTUM_RL 0.00075 // kg*m^2  // θ
#define ANGULAR_MOMENTUM_RR 0.00075 // kg*m^2  // θ

#define KV_FL 154.0 // motor_rpm/V
#define KV_FR 154.0 // motor_rpm/V
#define KV_RL 158.0 // motor_rpm/V
#define KV_RR 158.0 // motor_rpm/V

// #define CURRENT_TO_WHEEL_TORQUE_FL MOTOR_EFFICIENCY * 60 * GEAR_RATIO_FRONT / KV_FL / 2 / PI // Nm/A
// #define CURRENT_TO_WHEEL_TORQUE_FR MOTOR_EFFICIENCY * 60 * GEAR_RATIO_FRONT / KV_FR / 2 / PI // Nm/A
// #define CURRENT_TO_WHEEL_TORQUE_RL MOTOR_EFFICIENCY * 60 * GEAR_RATIO_REAR / KV_RL / 2 / PI  // Nm/A
// #define CURRENT_TO_WHEEL_TORQUE_RR MOTOR_EFFICIENCY * 60 * GEAR_RATIO_REAR / KV_RR / 2 / PI  // Nm/A

//#define REFFERENCE_BETA_MULTIPLIER_FL (1000000.0 * KV_RL *ANGULAR_MOMENTUM_RL) / (POLE_PAIR_COUNT * GEAR_RATIO_FRONT * GEAR_RATIO_REAR * MOTOR_EFFICIENCY) // don't ask this one!
//#define REFFERENCE_BETA_MULTIPLIER_FR (1000000.0 * KV_RR *ANGULAR_MOMENTUM_RR) / (POLE_PAIR_COUNT * GEAR_RATIO_FRONT * GEAR_RATIO_REAR * MOTOR_EFFICIENCY) // don't ask this one!
//#define REFFERENCE_BETA_MULTIPLIER_RL (1000000.0 * KV_FL *ANGULAR_MOMENTUM_FL) / (POLE_PAIR_COUNT * GEAR_RATIO_FRONT * GEAR_RATIO_REAR * MOTOR_EFFICIENCY) // don't ask this one!
//#define REFFERENCE_BETA_MULTIPLIER_RR (1000000.0 * KV_FR *ANGULAR_MOMENTUM_FR) / (POLE_PAIR_COUNT * GEAR_RATIO_FRONT * GEAR_RATIO_REAR * MOTOR_EFFICIENCY) // don't ask this one!
//
//#define BETA_MULTIPLIER_FL 1000000.0 * KV_FL *ANGULAR_MOMENTUM_FL / POLE_PAIR_COUNT / GEAR_RATIO_FRONT / GEAR_RATIO_FRONT / MOTOR_EFFICIENCY // this either
//#define BETA_MULTIPLIER_FR 1000000.0 * KV_FR *ANGULAR_MOMENTUM_FR / POLE_PAIR_COUNT / GEAR_RATIO_FRONT / GEAR_RATIO_FRONT / MOTOR_EFFICIENCY // this either
//#define BETA_MULTIPLIER_RL 1000000.0 * KV_RL *ANGULAR_MOMENTUM_RL / POLE_PAIR_COUNT / GEAR_RATIO_REAR / GEAR_RATIO_REAR / MOTOR_EFFICIENCY   // this either
//#define BETA_MULTIPLIER_RR 1000000.0 * KV_RR *ANGULAR_MOMENTUM_RR / POLE_PAIR_COUNT / GEAR_RATIO_REAR / GEAR_RATIO_REAR / MOTOR_EFFICIENCY   // this either

#define DIVIDER_FL 900 * MOTOR_EFFICIENCY * GEAR_RATIO_FRONT * POLE_PAIR_COUNT
#define DIVIDER_FR 900 * MOTOR_EFFICIENCY * GEAR_RATIO_FRONT * POLE_PAIR_COUNT
#define DIVIDER_RL 900 * MOTOR_EFFICIENCY * GEAR_RATIO_REAR * POLE_PAIR_COUNT
#define DIVIDER_RR 900 * MOTOR_EFFICIENCY * GEAR_RATIO_REAR * POLE_PAIR_COUNT
#define MULTIPLIER_FL KV_FL * PIPI * ANGULAR_MOMENTUM_FL
#define MULTIPLIER_FR KV_FR * PIPI * ANGULAR_MOMENTUM_FR
#define MULTIPLIER_RL KV_RL * PIPI * ANGULAR_MOMENTUM_RL
#define MULTIPLIER_RR KV_RR * PIPI * ANGULAR_MOMENTUM_RR


#define CHOOSE_TCS_IS_SLIDING_VERSION 2
#define CHOOSE_TCS_DONT_VERSION 2
#endif
