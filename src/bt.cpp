...
void bt_cmd(String cmd)
{
  ...
  else if (command == "t" || command == "tc" || command == "traction-control")
  { // traction control
    ...
    else
    {
      ...
      {
        bt_log("Traction control was enabled\n");
      }
      else
      {
        bt_log("Traction control was disabled\n");
      }
    }
  }
  else if (command == "p" || command == "pwm")
  { // pwm
    // if command[1] in enable_words:
    String second_word = cmd.substring(cmd.indexOf(' ') + 1);
    if (std::find(std::begin(enable_words), std::end(enable_words), second_word) !=
...
void pwm_status()
{
  if (en_pwm)
  {
    bt_log("Remote was enabled\n");
  }
  else
  {
    bt_log("Remote was disabled\n");
  }
}
